...
describe('humanize', function () {
  it('maps a known SHA-256 vector to custom words', function () {
    assert.strictEqual(
      humanize({ digest: '616263', wordlist: [list] }),
  ...
  it('supports separator and the legacy spelling, with separator taking precedence', function () {
    ...
    assert.strictEqual(
      humanize({ digest: hex, separator: '$', seperator: '/' }),
      expected,
    );
  });
  it('supports one word and the full 32-byte hash', function () {
    assert.strictEqual(
      humanize({ digest: hex, words: 1 }).split('-').length,
      1,
    );
    assert.strictEqual(
      humanize({ digest: hex, words: 32 }).split('-').length,
      32,
    );
  });
  it('uses all nine bits for 512-entry lists', function () {
    ...
    assert.strictEqual(
      humanize({ digest: '616263', wordlist: [large] }),
  ...
  it('rejects missing options and invalid word counts', function () {
    [undefined, null, 'abc', []].forEach((options) =>
      assert.throws(() => humanize(options), TypeError),
    );
  ...
  it('rejects requests beyond the hash bit budget', function () {
    const large = Array.from({ length: 512 }, (_, i) => 'word' + i);
    assert.throws(
      () => humanize({ digest: hex, words: 29, wordlist: [large] }),
      RangeError,
    );
  });
});
