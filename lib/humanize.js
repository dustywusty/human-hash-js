...
function humanize(options) {
  ...
  if (
    ...
    (wordlist.length !== 1 && wordlist.length !== words)
  ) {
    throw new TypeError(
      'wordlist must contain one list or one list per output word',
    );
  }
  for (const list of wordlist) {
    if (
      ...
      throw new RangeError(
        'Each word list must contain a power of two entries, from 2 to 65536',
      );
    }
    if (
      ...
      throw new TypeError(
        'Words must be nonempty strings without whitespace or the separator',
      );
    }
  ...
  const lists = Array.from(
    { length: words },
    (_, i) => wordlist[wordlist.length === 1 ? 0 : i],
  );
  ...
  return lists
    .map((list, i) => {
      ...
      for (let bit = 0; bit < widths[i]; bit++, offset++) {
        index =
          index * 2 +
          ((hash[Math.floor(offset / 8)] >>> (7 - (offset % 8))) & 1);
      }
      return list[index];
    })
    .join(separator);
