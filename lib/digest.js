...
module.exports = function parseDigest(digest) {
  if (typeof digest === 'string') {
    ...
    if (!/^(?:[0-9a-f]{2})+$/i.test(hex)) {
      throw new TypeError('digest must be nonempty, even-length hex or a hyphenated UUID');
    }
  ...
  if (Array.isArray(digest) && digest.length > 0 &&
      Array.from(digest).every(byte => Number.isInteger(byte) && byte >= 0 && byte <= 255)) {
    return Buffer.from(digest);
  }
  throw new TypeError('digest must be hex, a UUID, a Buffer, a Uint8Array, or an array of bytes');
};
